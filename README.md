...
c2.metric("Total de Divergências", df_graficos['Divergências'].sum() if not df_graficos.empty else 0, "Registradas", delta_color="inverse")
...
if unidade_selecionada == "Todas as Unidades":
    if not df_graficos.empty:
        ...
        c3.metric("Unidade em Alerta Máximo", "N/A")
else:
...
with col1:
    ...
    if len(df_graficos) > 2: 
        ...
        df_real = risco_diario.rename(columns={'y': 'Risco Real (R$ Mil)'}).set_index('Data')
